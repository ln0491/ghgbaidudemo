...
//        return r.height()*2;
    ...
    public static int calcTextHeight_(Paint paint, String demoText) {
...
//        return r.height();
    ...
    public static void drawXAxisValue(Canvas c, String text, float x, float y,
                                      Paint paint,
                                      PointF anchor, float angleDegrees, int index, int selectedIndex, XAxis xAxis,float recfWidth) {
...
//                if(index==5&&xAxis.lastAnchorX!=0){
//                    MyLog.log("xAxis.lastAnchorX="+xAxis.lastAnchorX);
//                    drawOffsetX -= mDrawTextRectBuffer.width() * xAxis.lastAnchorX;
//                }else{
                    drawOffsetX -= mDrawTextRectBuffer.width() * anchor.x;
//                }
                Log.d("vivi","mDrawTextRectBuffer.width() * anchor.x="+mDrawTextRectBuffer.width() * anchor.x);
                drawOffsetY -= lineHeight * anchor.y;
            }
            ...
            if (index==0&&xAxis.getFirstOffest()){
                drawOffsetX= drawOffsetX-mDrawTextRectBuffer.width() * anchor.x-convertDpToPixel(2);
            }

//            float textWidth=paint.measureText(text);
//            RectF rectF=new RectF(drawOffsetX-recfWidth,drawOffsetY-convertDpToPixel(13),drawOffsetX+recfWidth,drawOffsetY+convertDpToPixel(6));
//            RectF rectF=new RectF(drawOffsetX-convertDpToPixel(5),drawOffsetY-convertDpToPixel(13),drawOffsetX+paint.measureText(text)+convertDpToPixel(5),drawOffsetY+convertDpToPixel(6));
            RectF rectF=new RectF(drawOffsetX-(recfWidth-paint.measureText(text))/2,drawOffsetY-convertDpToPixel(13),drawOffsetX-(recfWidth-paint.measureText(text))/2+recfWidth,drawOffsetY+convertDpToPixel(6));
            if (index==selectedIndex){
                Paint p=new Paint();
                p.setAntiAlias(true);                       //设置画笔为无锯齿
                p.setColor(0xff444a5a);                    //设置画笔颜色
                p.setStrokeWidth(3.0f);              //线宽
                c.drawRect(rectF,p);
                ...
                Paint p=new Paint();
                p.setAntiAlias(true);                       //设置画笔为无锯齿
                p.setColor(Color.TRANSPARENT);                    //设置画笔颜色
                p.setStrokeWidth(3.0f);              //线宽
                c.drawRect(rectF,p);
                c.drawText(text, drawOffsetX, drawOffsetY, paint);
            }
//            xAxis.maps.put(index,(drawOffsetX-20)+"-"+(drawOffsetX+paint.measureText(text)+20)+"-"+(drawOffsetY-40));
            xAxis.maps.put(index, rectF.left+"-"+ rectF.top+"-"+rectF.right);
