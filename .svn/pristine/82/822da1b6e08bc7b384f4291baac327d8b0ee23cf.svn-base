...
public class DefaultClusterRenderer<T extends ClusterItem> implements
    ...
    private MyClusterRendered myClusterRendered;
     ...
     */
    private int min_cluster_size = 4;
    ...
    public DefaultClusterRenderer(Context context, BaiduMap map, ClusterManager<T> clusterManager, MyClusterRendered myClusterRendered) {
        mMap = map;
        mContext = context;
        this.myClusterRendered = myClusterRendered;
...
//            if (Constants.ClusterType.CLUSTER_TYPE==Constants.Area.STORE){
//                descriptor = BitmapDescriptorFactory.fromResource(R.mipmap.location);
//            }else {
                descriptor = BitmapDescriptorFactory.fromView(mIconGenerator.makeView(getClusterText(bucket)));
//            }
            mIcons.put(bucket, descriptor);
        }
        //        Toast.makeText(mContext,"bucket="+ bucket, Toast.LENGTH_SHORT).show();
    ...
    protected void onClusterRendered(Cluster<T> cluster, Marker marker) {
        myClusterRendered.onClusterRendered(Constants.ClusterType.CLUSTER_TYPE);
//          marker.setPosition(new LatLng(39.875466,116.470834));

//        LatLng latLng=cluster.getPosition();
//        Toast.makeText(mContext, "latitude="+latLng.latitude+"-longitude="+latLng.longitude, Toast.LENGTH_SHORT).show();
    }
...
//            if (Constants.ClusterType.ACTIVITY_TYPE == Constants.ClusterType.INDUSTRY) {
                if (Constants.ClusterType.CLUSTER_TYPE == Constants.Area.CITY||Constants.ClusterType.CLUSTER_TYPE ==Constants.Area.AREA||Constants.ClusterType.CLUSTER_TYPE ==Constants.Area.STORE) {
                    if (myClusterRendered != null) {
                        myClusterRendered.onBeforeClusterRendered(Constants.ClusterType.CLUSTER_TYPE);
                        return;
                    }
//                }
            }
            ...
            MarkerOptions markerOptions = new MarkerOptions()
                    .position(animateFrom == null ? new LatLng(39.875466,116.470834): animateFrom);
//            MarkerOptions markerOptions = new MarkerOptions()
//                    .position(animateFrom == null ? cluster.getPosition() : animateFrom);
            ...
            if (animateFrom != null) {
                markerModifier.animate(markerWithPosition, animateFrom, new LatLng(39.875466,116.470834));
//                markerModifier.animate(markerWithPosition, animateFrom, cluster.getPosition());
    ...
    public void setlusterSize(int clusterSize) {
        min_cluster_size = clusterSize;
    }

    public void initBusinessCompanyData(int proviceCount , String real_plan){
        mIconGenerator.initBusinessCompanyData(proviceCount,real_plan);
    }
    public void initIndustryData(String industryPlan){
        mIconGenerator.initIndustryData(industryPlan);
    }
    public void initBusinessData(String businessCount){
        mIconGenerator.initBusinessData(businessCount);
    }
