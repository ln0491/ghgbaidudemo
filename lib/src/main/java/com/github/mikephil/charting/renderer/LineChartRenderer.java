...
import com.github.mikephil.charting.utils.MyLog;
...
//
//                    Log.d("vivi","mLineBuffer["+i+"]"+mLineBuffer[i]);
//                }
            }
        }
...
//            float y = yVal * mAnimator.getPhaseY(); // get
            ...
            // y-position
//            if (xIndex>mXAxis.)
            float y=getYLabel(xIndex);
            String recf=mXAxis.maps.get(xIndex);
            if (recf!=null){
                String[]recfs=recf.split("-");
                float[] pts = new float[]{
                        xIndex, y
                };

                mChart.getTransformer(set.getAxisDependency()).pointValuesToPixel(pts);
                int lineWidth= (int) (Float.parseFloat(recfs[2])-Float.parseFloat(recfs[0]));
                // draw the lines
                drawHighlightLines(c, pts, set,lineWidth);
            }



//            try {
//                List<Entry> lists=set.getEntriesForXIndex(xIndex);
////                Collections.sort(lists, new Comparator<Entry>() {
////                    @Override
////                    public int compare(Entry lhs, Entry rhs) {
////                        return lhs.getVal()>=rhs.getVal()?1:-1;
////                    }
////                });
//                for (int x = 0; x < lists.size(); x++) {
//                    MyLog.log("list="+x+"---"+lists.get(x).toString());
//
//                }
//            }catch (Exception e){
//
...
//            String x=mXAxis.maps.get(xIndex);
//            String[] xx=x.split("-");
//            Paint paint=new Paint();
//            paint.setColor(0x7ff8dede);
//            RectF rectF=new RectF(Float.parseFloat(xx[0]),200,Float.parseFloat(xx[1]),Float.parseFloat(xx[2]));
//            Log.d("vivi","rectF="+(rectF.top-rectF.bottom));
...
//            c.drawRect(rectF);
    ...
    private float getYLabel(int xIndex){
        List<ILineDataSet> dataSets = mChart.getLineData().getDataSets();
//        Collections.sort(dataSets, new Comparator<ILineDataSet>() {
//            @Override
//            public int compare(ILineDataSet lhs, ILineDataSet rhs) {
//                return lhs.gete;
//            }
//        });
        List<Float>vals=new ArrayList<Float>();
        for (int i = 0; i < dataSets.size(); i++) {
            float val=dataSets.get(i).getEntryForXIndex(xIndex).getVal();
            vals.add(val);
        }
        ...
        List<Float>yLabels=new ArrayList<Float>();
        for (int i = 0; i < mYAxis.mEntryCount; i++) {
            String text = mYAxis.getFormattedLabel(i);
            MyLog.log("yLabels="+text);
            yLabels.add(Float.parseFloat(text));
        }
       float maxValueLabel= getMaxValueLabel(vals.get(vals.size()-1),yLabels);
        return  maxValueLabel;
    }

    private float getMaxValueLabel(float value,List<Float>labels){
        for (int i = 0; i < labels.size(); i++) {

            float label=labels.get(i);
            if (labels.get(i)>=value){
                return label;
            }
        }
        return 0;
...
//    protected void drawHighlightLines(Canvas c, float[] pts, ILineScatterCandleRadarDataSet set,int xIndex) {
//
//        MyLog.log("pts[0]="+pts[0]+"-pts[1]"+pts[1]+"-xIndex="+xIndex);
...
////        mHighlightPaint.setStrokeWidth(50);
//        // draw highlighted lines (if enabled)
////        mHighlightPaint.setPathEffect(set.getDashPathEffectHighlight());
//
//        // draw vertical highlight lines
//        if (set.isVerticalHighlightIndicatorEnabled()) {
//            String s=mXAxis.maps.get(xIndex);
//            if (s==null) return;
//            String ss[]=s.split("-");
//            MyLog.log("ss[0]="+ss[0]+"-ss[1]"+ss[1]+"-ss3="+ss[2]);
//            RectF rectF=new RectF(Float.parseFloat(ss[0]),Float.parseFloat(ss[1]),Float.parseFloat(ss[2]),pts[1]);
//            c.drawRect(rectF,mHighlightPaint);
////            mHighlightPaint.dr
//        }
//
