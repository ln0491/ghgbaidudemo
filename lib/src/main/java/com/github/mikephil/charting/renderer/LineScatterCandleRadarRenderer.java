...
public abstract class LineScatterCandleRadarRenderer extends DataRenderer {
    ...
    protected void drawHighlightLines(Canvas c, float[] pts, ILineScatterCandleRadarDataSet set) {
        ...
        mHighlightPaint.setColor(0x7ff4dadb);
...
//        }
     ...
     * @param set the currently drawn dataset
     */
    protected void drawHighlightLines(Canvas c, float[] pts, ILineScatterCandleRadarDataSet set,int lineWidth) {

        // set color and stroke-width
        mHighlightPaint.setColor(set.getHighLightColor());
        mHighlightPaint.setColor(0x7ff5f6f8);
//        mHighlightPaint.setColor(0x7ff4dadb);

        //        mHighlightPaint.setStrokeWidth(set.getHighlightLineWidth());
        mHighlightPaint.setStrokeWidth(lineWidth);
        // draw highlighted lines (if enabled)
        mHighlightPaint.setPathEffect(set.getDashPathEffectHighlight());
        ...
        if (set.isVerticalHighlightIndicatorEnabled()) {
            ...
            // create vertical path
            mHighlightLinePath.reset();
            mHighlightLinePath.moveTo(pts[0], pts[1]);
            mHighlightLinePath.lineTo(pts[0], mViewPortHandler.contentBottom());
